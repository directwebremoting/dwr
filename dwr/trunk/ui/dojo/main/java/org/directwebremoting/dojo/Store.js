...
dojo.declare("dwr.data.Store", null, {
    ...
    getAttributes : function(/* item */id) {
	    ...
	    var entry = this._entries[id];
	    if (entry == null)
		    throw new Error("non item passed to getAttributes()");
    ...
    isCached : function(request, baseResults) {
	    ...
	    for ( var i = 0; i < this._executedQueries.length; i++) {
		    ...
		    if (clientQuery !== false) {
			    var found = this.clientSideFetch({
			        start : 0,
			        count : Infinity,
			        query : request.query,
			    ...
			    if (request.start + request.count <= found) {
				    ...
				    var foundParent = this.clientSideFetch({
				        start : 0,
				        count : Infinity,
				        query : cachedRequest.query,
				        queryOptions : cachedRequest.queryOptions
				    }, baseResults).length;
    ...
    matchesQuery : function(item, request) {
	    ...
	    if (request.queryOptions && request.queryOptions.ignoreCase)
	    ...
	    for ( var i in request.query) {
		    ...
		    test = test ? !dojo.data.util.filter.patternToRegExp(match, ignoreCase).test(value) : value != match;
		    if (test) {
			    return false;
		    }
	    }
    ...
    _fetch : function(/* object */request) {
	    ...
	    var qOptions = request.queryOptions ? request.queryOptions : {
	        deep : false,
	        ignoreCase : false
	    };
    ...
    getLabelAttributes : function(/* item */id) {
	    ...
	    if (!this.isItem(id))
		    throw new Error("non item passed to getLabelAttributes()");
	    return [ "$label" ];
    },
    ...
    getIdentityAttributes : function(/* item */id) {
	    ...
	    if (!this.isItem(id))
		    throw new Error("non item passed to getIdentityAttributes()");
	    return [ "$id" ];
    },
    ...
    itemRemoved : function(/* StoreProvider */source, /* string */itemId) {
	    ...
	    if (dojo.isFunction(this.onDelete)) {
		    if (console && console.log)
			    console.log("Firing onDelete(", itemId, ",", label, ")");
    ...
    itemAdded : function(/* StoreProvider */source, /* Item */item) {
	    ...
	    if (this._entries[item.itemId] === undefined) {
		    ...
		    var valid = this.clientSideFetch({
		        start : 0,
		        count : Infinity,
		    ...
		    }, [ this._entries[item.$id] ]).length == 1;
    ...
    onSet : function(/* item */item, /* string */attribute, /* object|array */oldValue, /* object|array */newValue) {
	    ...
	    if (console && console.log)
		    console.log("Original onSet function called");
    },

    onNew : function(/* item */newItem, /* object? */parentInfo) {
	    // Summary: See `dojo.data.api.Notification.onNew`
	    if (console && console.log)
		    console.log("The store has been notified of an item [" + newItem + "] creation. Are you interested?");
    },

    onDelete : function(/* item */deletedItem) {
	    // Summary: See `dojo.data.api.Notification.onDelete`
	    if (console && console.log)
		    console.log("Original onDelete function called");
    },
    ...
    deleteItem : function(/* item */item) {
	    ...
	    if (entry == null)
		    throw new Error("non item passed to deleteItem(): " + item);
    ...
    setValue : function(/* item */item, /* string */attribute, /* anything */value) {
	    ...
	    var entry = this._entries[item];
	    if (entry == null)
		    throw new Error("non item passed to setValue()");
    ...
    setValues : function(/* item */item, /* string */attribute, /* array */values) {
	    ...
	    var entry = this._entries[item];
	    if (entry == null)
		    throw new Error("non item passed to setValues()");
    ...
    unsetAttribute : function(/* item */item, /* string */attribute) {
	    ...
	    var entry = this._entries[item];
	    if (entry == null)
		    throw new Error("non item passed to unsetAttribute()");
    ...
    isDirty : function(/* item? */item) {
	    ...
	    var entry = this._entries[item];
	    if (entry == null)
		    throw new Error("non item passed to isDirty()");
