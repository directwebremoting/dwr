...
/**
 * Given a map, or a recursive structure consisting of arrays and maps, call setValue() for all leaf entries and use intermediate levels to form nested element ids.
 *
...
dwr.util._setValuesRecursive = function(data, idpath, depth, options) {
	...
	// Object (not array) -> handle nested object properties
		...
		for ( var prop in data) {
			...
			if (dwr.util._isObject(data[prop]) && !dwr.util._isArray(data[prop]) && !dwr.util._isDate(data[prop]) || dwr.util._isArray(data[prop]) && data[prop].length > 0
...
/**
 * Given a map, or a recursive structure consisting of arrays and maps, call getValue() for all leaf entries and use intermediate levels to form nested element ids. Given a string or element that
 * refers to a form, create an object from the elements of the form.
 *
			...
			if (useOptions) {
				if (arg3 != null) {
					if (arg4 != null) {
						...
						options.text = options.value = dwr.util._getValueFrom(data[i], arg3);
				} else
					options.text = options.value = dwr.util._getValueFrom(data[i]);
...
/**
 * dwr.util.byId(ele).className -= "X", that we can call from Java easily From code originally by Gavin Kistner
 */
...
dwr.util._updateIds = function(ele, options) {
	...
	if (dwr.util._getId(ele)) {
		ele.setAttribute("id", ("idPrefix" in options ? options.idPrefix : "") + dwr.util._getId(ele) + ("idSuffix" in options ? options.idSuffix : ""));
	}
...
/**
 * Clone a template node and its embedded template child nodes according to cardinalities (of arrays) in supplied data.
 */
...
/**
 * @private Substitute a leading idpath fragment with another idpath for all element ids tree, and remove ids that don't match the idpath.
 */
 ...
 * @param nodeName eg "input", "textarea" - check for node name (optional) if nodeName is an array then check all for a match.
 */
...
dwr.util._isArrayLike = function(data) {
	return data && (typeof data.length == "number") // must have .length
...
dwr.util._isFunction = function(data) {
	return (data && (typeof data == "function" || Object.prototype.toString.call(data) == "[object Function]") && data != "[object NodeList]"); // need to workaround NodeList on Safari
};
