...
(function() {
	...
	/**
	 * Setter for the text/html handler - what happens if a DWR request gets an HTML reply rather than the expected Javascript. Often due to login timeout
	 *
	...
	/**
	 * Setter for the html response/redirect handler - what happens if a DWR request gets an HTML reply or a redirect status code rather than the expected Javascript. Often due to login timeout
	 *
	 * @param {Function} handler The function to call on an unexpected text/html content type
	 */
	...
	/**
	 * Do we ask the XHR object to be asynchronous? (Default: true) Warning: it is <strong>highly</strong> advised to use the default of async processing, especially when dealing with Internet based
	 * requests.
	 *
	...
	/*
	 * The maximum number of retries, when reached we will no longer attempt to check for server status. @param - maxRetries
	 */
	...
	/*
	 * The intervals between successive retries in seconds @param - array of integers representing the retry interval in seconds.
	 */
	...
	/**
	 * For use with file downloads. When a DWR function returns a binary download you can prompt the user to save it using this function
	 *
	...
	/**
	 * What is the current version DWR number DWR version numbers are of the form "Version 1.2.3.3128[.beta]", where: 1 is the major release number. Changes in major version number indicate
	 * significant enhancements in functionality 2 is the minor release number. Changes in minor version number indicate less significant changes in functionality 3 is the revision release number.
	 * Changes here typically indicate bug fixes only 3128 is the build number. This number increments for each build .beta is a release title that is generally only used for non production releases
	 * to indicate the purpose/quality of the release The label is these strings concatenated
	 */
	dwr.version = {
	    /**
	    ...
	    /**
		 * Only used for non production releases to indicate the purpose/quality of the release. Example titles include 'milestone1' or 'beta3'.
		 */
	    title : "${versionTitle}",
	...
	/**
	 * The default path to the DWR servlet pathToDwrServlet is aids cross-domain. If pathToDwrServlet is defined before engine.js is included pathToDwrServlet will be used.
	 */
	...
	/**
	 * Used as the default for reverse ajax/polling Retry immediately twice with one second intervals between, then go offline. Retry every 10 seconds when offline.
	 */
	 ...
	 * @param func The callback function to which any returned data should be passed if this is null, any returned data will be ignored
	...
	/**
	 * Is this a batch that is checking for a heartbeat? If so we want to ignore errors/warnings, etc.
	 *
	 * @private
	 * @param {Object} batch
	 */
	...
	dwr.engine._handleTextHtmlResponse = function(batch, textHtmlObj) {
		...
		} else if (dwr.engine._textHtmlHandler) {
	...
	dwr.engine._handleTextOrRedirectResponse = function(batch, textHtmlObj) {
		...
		} else if (dwr.engine._textOrRedirectHandler) {
	...
	/**
	 * Navigates properties from the global scope and down to fetch a property value.
	 *
	...
	dwr.engine.serialize = {
		 ...
		 */
	    domDocument : [ "Msxml2.DOMDocument.6.0", "Msxml2.DOMDocument.5.0", "Msxml2.DOMDocument.4.0", "Msxml2.DOMDocument.3.0", "MSXML2.DOMDocument", "MSXML.DOMDocument",
	            "Microsoft.XMLDOM" ],
	    ...
	    /**
		 * Returns the classname of supplied argument obj. Similar to typeof, but which returns the name of the constructor that created the object rather than 'object'
		 *
		 * @private
	...
	dwr.engine.transport = {
	    ...
	    xhr : {
	        ...
	        /**
			 * The ActiveX objects to use when we want to do an XMLHttpRequest call. TODO: We arrived at this by trial and error. Other toolkits use different strings, maybe there is an officially
			 * correct version?
			 */
			        ...
			        if (status >= 200 && status < 300) {
				        ...
				        if (!contentType.match(/^text\/plain/) && !contentType.match(/^text\/javascript/)) {
					        if (contentType.match(/^text\/html/)) {
						        dwr.engine._handleTextHtmlResponse(batch, {
						            status : status,
						            responseText : reply,
				        ...
				        dwr.engine._handleTextOrRedirectResponse(batch, {
				            status : status,
				            responseText : reply,
				            contentType : contentType
				        });
	        ...
	        /**
			 * Some more text might have come in, test and execute the new stuff. This method could also be called by the iframe transport
			 *
			 * @private
	    ...
	    iframe : {
	        ...
	        send : function(batch) {
		        ...
		        batch.div1.innerHTML = "<iframe src='" + dwr.engine.SSL_SECURE_URL + "' frameborder='0' style='width:0px;height:0px;border:0;display:none;' id='" + idname
		                + "' name='" + idname + "'></iframe>";
	        ...
	        /**
			 * Setup a form or construct a src attribute to use the iframe. This is abstracted from send() because the same logic will do for htmlfile
			 *
			 * @param {Object} batch
			 */
	    ...
	    /*
		 * // If we have an iframe comet solution where we need to read data streamed // into an iframe then we need code like this to slurp the data out. // Compare this with xhr.checkCometPoll()
		 * outstandingIFrames:[],
		 *
		 * checkCometPoll:function() { for (var i = 0; i < dwr.engine.transport.iframe.outstandingIFrames.length; i++) { var text = ""; var iframe = dwr.engine.transport.iframe.outstandingIFrames[i];
		 * try { text = dwr.engine.transport.iframe.getTextFromCometIFrame(iframe); } catch (ex) { dwr.engine._handleWarning(iframe.batch, ex); } if (text != "")
		 * dwr.engine.transport.xhr.processCometResponse(text, iframe.batch); }
		 *
		 * if (dwr.engine.transport.iframe.outstandingIFrames.length > 0) { setTimeout(dwr.engine.transport.iframe.checkCometPoll, dwr.engine._pollCometInterval); } } // We probably also need to
		 * update dwr.engine.remote.beginIFrameResponse() // to call checkCometPoll. // Extract the whole (executed and all) text from the current iframe getTextFromCometIFrame:function(frameEle) {
		 * var body = frameEle.contentWindow.document.body; if (body == null) return ""; var text = body.innerHTML; // We need to prevent IE from stripping line feeds if (text.indexOf("<PRE>") == 0 ||
		 * text.indexOf("<pre>") == 0) { text = text.substring(5, text.length - 7); } return text; }; // And an addition to iframe.remove(): { if (batch.iframe) { // If this is a poll frame then
		 * stop comet polling for (var i = 0; i < dwr.engine.transport.iframe.outstandingIFrames.length; i++) { if (dwr.engine.transport.iframe.outstandingIFrames[i] == batch.iframe) {
	    ...
	    htmlfile : {
		    ...
		    send : function(batch) {
			    ...
			    batch.htmlfile.write("<div><iframe className='wibble' src='javascript:void(0)' id='" + idname + "' name='" + idname
			            + "' onload='dwr.engine.transport.iframe.loadingComplete(" + batch.map.batchId + ");'></iframe></div>");
			    batch.htmlfile.write("</" + "html>");
	...
	dwr.engine.batch = {
	    ...
	    constructRequest : function(batch, httpMethod) {
		    ...
		    var sessionMatchExpr = new RegExp("^" + // start of string
	    ...
	    /**
		 * @private This function is invoked when a batch reply is received. It checks that there is a response for every call in the batch. Otherwise, an error will be signaled (a call without a
		 *          response indicates that the server failed to send complete batch response).
		 */
	...
	dwr.engine.util = {
	    ...
	    isCallOptionArgument : function(lastArg) {
		    return (typeof lastArg === "object" && (typeof lastArg.callback === "function" || typeof lastArg.exceptionHandler === "function"
		            || typeof lastArg.callbackHandler === "function" || typeof lastArg.errorHandler === "function" || typeof lastArg.warningHandler === "function" || lastArg
	...
	/**
	 * High level data-sync API for use by Widget libraries like a Dojo-Data-Store. For full documentation see org.directwebremoting.export.Data
	 */
	dwr.data = {

	    /**
		 * This is just documentation that defines how the listener parameter must act in order to receive asynchronous updates
		 */
	    StoreChangeListener : {
	        /**
			 ...
			 * @param {StoreProvider} source The store from which it was moved
			 * @param {Item} item The thing that has changed
			 * @param {string[]} changedAttributes A list of changed attributes. If null then you should assume that everything has changed
			 */
	    ...
	    /**
		 * Create a cache object containing the functions to interact with a server side StoreProvider
		 *
		 * @param {string} storeId ID of server provided storage
		 * @param {dwr.data.StoreChangeListener} listener See server documentation This is likely to be true if dwr.engine.activeReverseAjax == true
		 */
	 ...
	 *
	 * @param {Object} region filtering and sorting options. Includes: - start: The beginning of the region of specific interest - count: The number of items being viewed - sort: The sort criteria -
